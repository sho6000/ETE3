...
class ImageProcessor:
    ...
    def add_watermark(self, image, text, position=(10, 10), font_size=30, color=(255, 255, 255, 128)):
    ...
    def save_processed_image(self, image, day, track, suffix):
        # Create filename
    ...
    def display(self):
        ...
        if uploaded_file is not None:
            ...
            selected_track = st.selectbox("Select Track", self.df['Track'].unique())
            ...
            with col1:
                if st.button("Apply Grayscale"):
                    st.session_state.processed_image = self.apply_grayscale(opencv_image)
            ...
            with col2:
                if st.button("Apply Blur"):
                    st.session_state.processed_image = self.apply_blur(opencv_image)
                    st.session_state.filter_applied = True
            ...
            with col3:
                if st.button("Apply Edge Detection"):
                    st.session_state.processed_image = self.apply_edge_detection(opencv_image)
                    st.session_state.filter_applied = True
            ...
            if st.session_state.filter_applied or st.session_state.watermark_applied:
                ...
                st.markdown(self.get_image_download_link(processed_pil, filename, "Download Processed Image"), unsafe_allow_html=True)
        ...
        image_files = self.get_image_files(day_dir)
        ...
        if not image_files:
            ...
            for i, img_path in enumerate(image_files):
                with cols[i % 3]:
                    ...
                    track = "Unknown Track"
                    for t in self.df['Track'].unique():
                        if t.lower() in name_without_ext.lower():
                            track = t
                            break
                    
                    # Display image with caption
                    self.display_image_with_caption(img_path, f"Day {selected_day} - {track}") 